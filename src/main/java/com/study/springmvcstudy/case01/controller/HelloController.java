...
public class HelloController {
	...
	/*
	 * 3. 帶參數 Lab 練習：?後帶入參數並計算 路徑：/bmi?h=170.0&w=60.0 可以使用：Optional<?> 來接參數
	 */
	...
	/*
	 * 5. @RequestParam + @PathVariable (Lab 練習) 路徑：/calc/add?x=30&y=20 -> Result：30
	 * + 20 = 50 路徑：/calc/sub?x=30&y=20 -> Result：30 - 20 = 10 路徑：/calc/sub?y=20 ->
	 * Result： 0 - 20 = -20 路徑：/calc/add -> Result： 0 + 0 = 0 路徑：/calc/div ->
	...
	/*
	 * 6. @PathVariable (萬用字元： * 任意多字、? 任意一字) 路徑：/path/namejohn/java8
	 * 路徑：/path/nameTaipei/java7 路徑：/path/name1234/java6
	 */
	...
	public String javaexam(@RequestParam("score") List<Integer> scores) {
		...
		return String.format("height: %d, low: %d, avg: %.2f, sum: %d", stat.getMax(), stat.getMin(), stat.getAverage(),
				stat.getSum());
	}
	...
	/*
	 * 10. Map 參數 路徑：/person?name=John&score=100&age=18&pass=true
	 * 路徑：/person?name=Mary&score=90&level=2 常與於各 form 表單傳來的不統一參數
	 */
